...
func HandleLogGet(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		response.WithJSON(w, http.StatusInternalServerError, "Failed to read log")
		return
	}
	defer file.Close()
	...
	if err != nil {
		response.WithJSON(w, http.StatusInternalServerError, "Failed to read log content")
		return
	}

	response.WithJSON(w, http.StatusOK, string(c[:]))
}
...
func HandleLogPost(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		response.WithJSON(w, http.StatusInternalServerError, "Failed to read response body")
		return
	}
	defer r.Body.Close()
	...
	if strings.TrimSpace(content) == "" {
		response.WithJSON(w, http.StatusInternalServerError, "Cannot save empty log")
		return
	}

	file, err := os.OpenFile(path+"/log.txt", os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0644)
	if err != nil {
		response.WithJSON(w, http.StatusInternalServerError, "Failed to create log file")
		return
	}
	defer file.Close()
	...
	if err != nil {
		response.WithJSON(w, http.StatusInternalServerError, "Failed to write to log file")
		return
	}
